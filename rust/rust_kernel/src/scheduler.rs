    ...
    }
    {
        ...
        unsafe {
            ...
            *(user_stack_top as *mut u32) = 0xDEADBEEF;
            ...
            esp -= 4;
            *(esp as *mut u32) = 0x23; // SS
            esp -= 4;
            *(esp as *mut u32) = user_stack_top; // User ESP (top of NEW user stack)
            esp -= 4;
            *(esp as *mut u32) = 0x202; // EFLAGS (IF=1)
            esp -= 4;
            *(esp as *mut u32) = 0x1B; // CS
            esp -= 4;
            *(esp as *mut u32) = entry_point; // EIP

            // Error code and interrupt number (skipped by add esp,8)
            esp -= 4;
            *(esp as *mut u32) = 0; // err_code
            esp -= 4;
            *(esp as *mut u32) = 32; // int_no
            ...
            *(esp as *mut u32) = 0; // EDI
            ...
            esp -= 4;
            *(esp as *mut u32) = 0x23; // DS
            esp -= 4;
            *(esp as *mut u32) = 0x23; // ES
            esp -= 4;
            *(esp as *mut u32) = 0x23; // FS
            esp -= 4;
            *(esp as *mut u32) = 0x23; // GS
...
pub extern "C" fn rust_spawn_process_from_file(path_ptr: *const u8) {
    let _path = unsafe {
        core::ffi::CStr::from_ptr(path_ptr as *const core::ffi::c_char)
            .to_str()
            .unwrap()
    };
